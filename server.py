...
def send_mining_notify_to_all():
    for miner in mining.miner_conns[:]:
        thread = threading.Thread(target=stratum.send_mining_notify,
                                  args=(miner, 'null'))
        thread.start()
...
def calc_block_pow(part1, payload, part3, timestamp, nonce):
    ...
    new_timestamp = timestamp[6:] + timestamp[4:6] + timestamp[
        2:4] + timestamp[0:2]
...
def get_block_pow(payload, timestamp, nonce):
    pow1 = calc_block_pow(client.last_miner_notify[1]["params"][0]["part1"],
                          payload,
                          client.last_miner_notify[1]["params"][0]["part3"],
                          timestamp, nonce)
    if client.last_miner_notify_buf_full:
        pow2 = calc_block_pow(
            client.last_miner_notify[0]["params"][0]["part1"], payload,
            client.last_miner_notify[0]["params"][0]["part3"], timestamp,
            nonce)
    else:
        return pow1
...
def connection_handler(conn, addr, difficulty):
    ...
    while True:
        try:
            ...
            print("Miner disconnected from " + new_miner.addr[0] + ':' +
                  str(new_miner.addr[1]))
            try:
        ...
        if buf:
            ...
            for msg in msgs:
                ...
                if msg["method"] == "mining.subscribe":
                    extranonce = stratum.send_subscribe_ack(
                        new_miner, msg["id"])
                    ...
                    try:
                        ...
                        try:
                            accountancy.account_fees[account] = int(
                                msg["params"][1])
                        except:
                            accountancy.account_fees[
                                account] = accountancy.pool_fee
                        if accountancy.account_fees[
                                account] < accountancy.pool_fee or accountancy.account_fees[
                                    account] > 100:
                            accountancy.account_fees[
                                account] = accountancy.pool_fee
                    except:
                    ...
                    if account not in mining.miners:
                        ...
                        mining.miners[
                            account] = 0  #set shares to 0 for that account
                    stratum.send_auth_ack(new_miner, msg["id"])

                elif msg["method"] == "mining.submit":
                    payload = extranonce + msg["params"][2]
                    if last_payload == payload and last_timestamp == msg[
                            "params"][3] and last_nonce == msg["params"][4]:
                        stratum.send_submit_error(new_miner, msg["id"])
                        #print("Share rejected because of duplication from: " + new_miner.addr[0])
                    else:
                        block_pow = get_block_pow(payload, msg["params"][3],
                                                  msg["params"][4])
                        target_pow = int(
                            client.last_miner_notify[
                                client.last_miner_notify_cnt]["params"][0]
                            ["target_pow"], 16)
                        ...
                        if calc_diff_from_target(block_pow) >= difficulty:
                            ...
                            timestamp = time.time()
                            mining.shares[account].add_share(
                        ...
                        if block_pow <= target_pow:
                            print("Block found with this share from: " +
                                  new_miner.addr[0])
...
def start_diff_servers():
    ...
    for i in range(len(diffs)):
        thread_server = threading.Thread(target=server_handler,
                                         args=(diffs[i], diff_ports[i]))
        thread_server.start()
